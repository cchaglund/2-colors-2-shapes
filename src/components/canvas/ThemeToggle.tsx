...
export function ThemeToggle({ mode, onSetMode, theme, onSetTheme }: ThemeToggleProps) {
  return (
    <div className="flex flex-col gap-3">
      {/* Dark mode toggle */}
      <div>
        <h4 className="m-0 mb-1 text-xs uppercase text-(--color-text-tertiary)">Mode</h4>
      ...
      <div>
        <h4 className="m-0 mb-1 text-xs uppercase text-(--color-text-tertiary)">Theme</h4>
        <div className="flex gap-1">
          {THEMES.map((t) => (
            <button
              key={t}
              className={`flex-1 py-1.5 px-2 rounded text-xs font-semibold uppercase transition-colors ${
                theme === t
                  ? 'bg-(--color-selected) text-(--color-accent) border border-(--color-accent)'
                  : 'bg-(--color-bg-tertiary) text-(--color-text-secondary) border border-transparent hover:bg-(--color-hover)'
              }`}
