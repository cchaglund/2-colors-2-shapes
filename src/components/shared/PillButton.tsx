...
export function PillButton<T extends ElementType = 'button'>({
  ...
  return (
    <Tag
      className={`h-9 md:h-8 px-3 rounded-(--radius-pill) text-(--text-xs) font-semibold transition-colors cursor-pointer inline-flex items-center justify-center no-underline ${fullWidth ? 'w-full' : ''} ${variantClasses[variant]} ${className}`}
      style={{
